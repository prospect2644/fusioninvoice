...
class InvoiceCalculate
{
    public function calculate($invoice)
    {
        ...
        $totalPaid = PaymentInvoice::where('invoice_id', $invoice->id)->sum('invoice_amount_paid');
        ...
        if ($calculatedAmount['total'] > 0 and $calculatedAmount['balance'] > 0 and $invoice->status == 'paid')
        {
            $invoice->status = 'sent';
            $invoice->save();
        }
