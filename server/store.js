...
const emptyStore = { clients: [], invoices: [], estimates: [], payments: [], items: [], subscriptions: [], subscriptionInvoiceRuns: [], expenses: [], tasks: [], tickets: [], documentFolders: [], documents: [], settings: { customFields: [] } };
...
export function addTicket(input) {
  ...
  const board=input.board||'technical_support',remaining=item=>Number(item.hourlyAllotment||0)*60-data.tickets.filter(ticket=>ticket.subscriptionId===item.id).flatMap(ticket=>ticket.timeEntries||[]).reduce((sum,entry)=>sum+Number(entry.minutes||0),0),managed=board==='technical_support'?data.subscriptions.find(item=>item.clientId===input.clientId&&item.managedIt&&item.status==='active'&&remaining(item)>0):null,ticket={id:nextNumber(data.tickets,'TKT',1001),clientId:input.clientId,contactName:String(input.contactName||'').trim(),contactEmail:String(input.contactEmail||'').trim(),title:String(input.title||'').trim(),board,status:input.status||'open',billingType:managed?'subscription':board==='technical_support'?'hourly':input.billingType||'hourly',subscriptionId:managed?.id||(board==='technical_support'?null:input.subscriptionId||null),hourlyRate:Number(input.hourlyRate||client.hourlyRate||0),closedAt:null,notes:[],timeEntries:[],createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
  if(!ticket.contactName||!ticket.title||!['technical_support','projects','maintenance'].includes(ticket.board)||!['open','in_progress','waiting_customer','waiting_vendor','closed'].includes(ticket.status)||!['hourly','subscription'].includes(ticket.billingType))throw new Error('Company, contact, title, board, billing method, and valid status are required.');if(ticket.billingType==='subscription'&&!data.subscriptions.some(item=>item.id===ticket.subscriptionId&&item.clientId===ticket.clientId))throw new Error('Choose a subscription belonging to this company.');if(ticket.status==='closed')ticket.closedAt=new Date().toISOString();data.tickets.unshift(ticket);write(data);return ticket;
}
