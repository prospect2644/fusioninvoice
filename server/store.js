...
export function updateTicketClassification(id,input){const data=read(),ticket=data.tickets.find(item=>item.id===id),board=String(input.board||''),validBoard=data.settings.ticketBoards.find(item=>item.id===board);if(!ticket)throw new Error('Ticket not found.');if(!validBoard)throw new Error('Choose a valid service board.');const subscription=data.subscriptions.find(item=>item.id===ticket.subscriptionId);if(subscription?.managedIt&&!(subscription.boardIds||[]).includes(board))throw new Error('The ticket subscription does not cover that service board. Change billing first or choose a covered board.');const category=data.settings.ticketCategories.filter(item=>item.boardId===board).sort((a,b)=>(a.position||0)-(b.position||0))[0];if(!category)throw new Error('Add a category to this service board before assigning tickets to it.');Object.assign(ticket,{board,category:category.id,updatedAt:new Date().toISOString()});write(data);return ticket;}
