...
async function api(request, env) {
  ...
  if (method === 'POST' && path === '/api/tickets') {
    const clientId=String(body.clientId||''),contactId=String(body.contactId||'')||null,title=String(body.title||'').trim(),status=String(body.status||'open'),billingType=String(body.billingType||'hourly'),subscriptionId=String(body.subscriptionId||'')||null,hourlyRate=cents(body.hourlyRate||0);let contactName=String(body.contactName||'').trim(),contactEmail=String(body.contactEmail||'').trim();
    if(contactId){const contact=await env.DB.prepare('SELECT name,email FROM client_contacts WHERE id = ? AND client_id = ? AND workspace_id = ?').bind(contactId,clientId,workspaceId).first();if(!contact)return json({error:'Choose a contact belonging to this company.'},400);contactName=contact.name;contactEmail=contact.email;}
