...
async function ticketBillableCents(db,workspaceId,ticketId){
  const ticket=await db.prepare('SELECT billing_type,subscription_id,hourly_rate_cents FROM tickets WHERE id = ? AND workspace_id = ?').bind(ticketId,workspaceId).first();
  ...
  if(ticket.billing_type==='subscription'&&ticket.subscription_id){
    ...
    const entries=await all(db.prepare('SELECT e.ticket_id,e.minutes FROM ticket_time_entries e JOIN tickets t ON t.id=e.ticket_id WHERE t.subscription_id = ? AND t.workspace_id = ? ORDER BY e.created_at,e.id').bind(ticket.subscription_id,workspaceId));
    const allotment=Number(subscription?.hourly_allotment_minutes||0);let used=0;billableMinutes=0;
...
async function api(request, env) {
  ...
  await Promise.all([ensureInvoiceWorkflowSchema(env.DB),ensureDocumentationSchema(env.DB),ensureTicketCommunicationSchema(env.DB)]);
  ...
  if(method==='PATCH'&&subscriptionRecord){const id=decodeURIComponent(subscriptionRecord[1]),clientId=String(body.clientId||''),summary=String(body.summary||'').trim(),nextDate=String(body.nextDate||''),stopDate=String(body.stopDate||'')||null,intervalCount=Number(body.intervalCount||1),intervalUnit=String(body.intervalUnit||'months'),amount=cents(body.amount||0),managedIt=body.managedIt==='on'||body.managedIt===true?1:0,hourlyAllotmentMinutes=Math.round(Number(body.hourlyAllotment||0)*60),status=String(body.status||'active');if(!clientId||!summary||!nextDate||!(intervalCount>0)||!Number.isInteger(intervalCount)||!['days','weeks','months','years'].includes(intervalUnit)||!(amount>=0)||!Number.isInteger(hourlyAllotmentMinutes)||hourlyAllotmentMinutes<0||!['active','paused','ended'].includes(status))return json({error:'Complete the subscription details with a valid recurrence, allotment, and amount.'},400);if(!await env.DB.prepare('SELECT id FROM clients WHERE id = ? AND workspace_id = ?').bind(clientId,workspaceId).first())return json({error:'Client not found.'},404);const result=await env.DB.prepare('UPDATE subscriptions SET client_id = ?, summary = ?, next_date = ?, stop_date = ?, interval_count = ?, interval_unit = ?, amount_cents = ?, managed_it = ?, hourly_allotment_minutes = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND workspace_id = ?').bind(clientId,summary,nextDate,stopDate,intervalCount,intervalUnit,amount,managedIt,hourlyAllotmentMinutes,status,id,workspaceId).run();if(!result.meta.changes)return json({error:'Subscription not found.'},404);const invoicesCreated=await generateSubscriptionInvoices(env.DB,workspaceId);return json({id,invoicesCreated});}
