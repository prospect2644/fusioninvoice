...
async function readWorkspace(db, workspaceId, identity) {
  const [clients, clientAddresses, invoices, invoiceItems, estimates, estimateItems, payments, items, subscriptions, expenses, tasks, tickets, ticketNotes, ticketTime, ticketEmailPreferences, ticketNoteDelivery, assets, accounts, contacts, fields, values, documentFolders, documents] = await Promise.all([
...
async function api(request, env) {
  ...
  if(method==='PATCH'&&documentRecord){const id=decodeURIComponent(documentRecord[1]),title=String(body.title||'').trim(),folderId=String(body.folderId||'')||null;if(!title||title.length>180)return json({error:'Enter a document title up to 180 characters.'},400);if(folderId&&!await env.DB.prepare('SELECT id FROM document_folders WHERE id = ? AND workspace_id = ?').bind(folderId,workspaceId).first())return json({error:'Folder not found.'},404);const result=await env.DB.prepare('UPDATE documents SET title = ?, folder_id = ?, content_html = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND workspace_id = ?').bind(title,folderId,cleanDocumentHtml(body.contentHtml),id,workspaceId).run();if(!result.meta.changes)return json({error:'Document not found.'},404);return json({id});}
