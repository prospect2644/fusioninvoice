...
async function readWorkspace(db, workspaceId, identity) {
  const [clients, invoices, invoiceItems, estimates, estimateItems, payments, items, subscriptions, expenses, tasks, tickets, ticketNotes, ticketTime, assets, accounts, contacts, fields, values] = await Promise.all([
    ...
    optionalAll(db.prepare('SELECT id, client_id, contact_id, contact_name, contact_email, title, status, billing_type, subscription_id, hourly_rate_cents, closed_at, created_at, updated_at FROM tickets WHERE workspace_id = ? ORDER BY updated_at DESC').bind(workspaceId)),
...
async function api(request, env) {
  ...
  if(method==='PATCH'&&clientUpdate){const id=decodeURIComponent(clientUpdate[1]),name=String(body.name||'').trim(),city=String(body.city||'').trim(),state=String(body.state||'').trim().toUpperCase(),zip=String(body.zip||'').trim(),hourlyRate=cents(body.hourlyRate||0),status=String(body.status||'active');if(!name||!city||!state||!zip||state.length>2||!Number.isFinite(hourlyRate)||hourlyRate<0||!['active','inactive'].includes(status))return json({error:'Enter a valid name, city, state, ZIP, hourly rate, and status.'},400);const result=await env.DB.prepare('UPDATE clients SET name = ?, city = ?, state = ?, zip = ?, hourly_rate_cents = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND workspace_id = ?').bind(name,city,state,zip,hourlyRate,status,id,workspaceId).run();if(!result.meta.changes)return json({error:'Client not found.'},404);return json({id});}
  ...
  if (method === 'POST' && path === '/api/tickets') {
    const clientId=String(body.clientId||''),contactId=String(body.contactId||''),title=String(body.title||'').trim(),status=String(body.status||'open'),billingType=String(body.billingType||'hourly'),subscriptionId=String(body.subscriptionId||'')||null,hourlyRate=cents(body.hourlyRate||0);
    if(!clientId||!contactId||!title||!['open','in_progress','waiting_customer','waiting_vendor','closed'].includes(status)||!['hourly','subscription'].includes(billingType)||hourlyRate<0)return json({error:'Company, saved contact, title, billing method, and valid status are required.'},400);
    ...
    await env.DB.prepare('INSERT INTO tickets (id, workspace_id, client_id, contact_id, contact_name, contact_email, title, status, billing_type, subscription_id, hourly_rate_cents, closed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').bind(id,workspaceId,clientId,contactId,contactName,contactEmail,title,status,billingType,subscriptionId,hourlyRate,closedAt).run();return json({id},201);
  }
